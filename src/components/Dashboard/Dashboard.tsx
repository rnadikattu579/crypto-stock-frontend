    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 hover:shadow-xl transition-all">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Total Value</p>
                <p className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1 sm:mt-2 transition-all duration-300 truncate">
              ...
              <div className="p-2 sm:p-3 bg-blue-100 dark:bg-blue-900/30 rounded-full flex-shrink-0 ml-2">
                <DollarSign className="h-6 w-6 sm:h-8 sm:w-8 text-blue-600 dark:text-blue-400" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 hover:shadow-xl transition-all">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Total Gain/Loss</p>
                <p className={`text-2xl sm:text-3xl font-bold mt-1 sm:mt-2 transition-all duration-300 truncate ${isPositive ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  {isPositive ? '+' : ''}${displaySummary?.total_gain_loss.toFixed(2) || '0.00'}
                </p>
                <p className={`text-xs sm:text-sm font-semibold transition-all duration-300 ${isPositive ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  {displaySummary?.total_gain_loss_percentage.toFixed(2)}%
                </p>
              </div>
              <div className={`p-2 sm:p-3 rounded-full transition-all duration-300 flex-shrink-0 ml-2 ${isPositive ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30'}`}>
                {isPositive ? (
          ...
          <div
            ...
            className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 hover:shadow-xl transition-all cursor-pointer transform hover:scale-105 active:scale-95"
          >
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Crypto Value</p>
                <p className="text-2xl sm:text-3xl font-bold text-purple-600 dark:text-purple-400 mt-1 sm:mt-2 transition-all duration-300 truncate">
                  ${displaySummary?.crypto_value.toFixed(2) || '0.00'}
                </p>
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 transition-all duration-300">{displaySummary?.crypto_count || 0} assets</p>
              </div>
              <div className="p-2 sm:p-3 bg-purple-100 dark:bg-purple-900/30 rounded-full flex-shrink-0 ml-2">
                <PieChart className="h-6 w-6 sm:h-8 sm:w-8 text-purple-600 dark:text-purple-400" />
              </div>
            </div>
          </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">Stock Value</p>
                <p className="text-2xl sm:text-3xl font-bold text-indigo-600 dark:text-indigo-400 mt-1 sm:mt-2 transition-all duration-300 truncate">
                  ${displaySummary?.stock_value.toFixed(2) || '0.00'}
                </p>
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 transition-all duration-300">{displaySummary?.stock_count || 0} assets</p>
              </div>
              <div className="p-2 sm:p-3 bg-indigo-100 dark:bg-indigo-900/30 rounded-full flex-shrink-0 ml-2">
                <PieChart className="h-6 w-6 sm:h-8 sm:w-8 text-indigo-600 dark:text-indigo-400" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white mb-3 sm:mb-4">Portfolio Distribution</h2>
                  ...
                  <Pie
                    ...
                    label={({ name, percent, value }) => {
                      ...
                      if (window.innerWidth < 640) {
                        return `${name.substring(0, 3)}: ${(percent * 100).toFixed(0)}%`;
                      }
                      return `${name}: $${value.toFixed(2)} (${(percent * 100).toFixed(0)}%)`;
                    }}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white mb-3 sm:mb-4">Top Performers</h2>
            {performanceData.length > 0 ? (
              <ResponsiveContainer width="100%" height={250} className="sm:h-[300px]">
                <BarChart data={performanceData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                  ...
                  />
                  <YAxis
                    tickFormatter={(value) => window.innerWidth < 640 ? `$${Math.round(value)}` : `$${value.toFixed(0)}`}
                    tick={{ fontSize: window.innerWidth < 640 ? 10 : 12 }}
                  />
                  <Tooltip
                    content={({ active, payload }) => {
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="bg-white dark:bg-gray-700 p-2 sm:p-3 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg">
                            <p className="font-semibold text-sm sm:text-base text-gray-900 dark:text-white">{data.name}</p>
                            <p className={`text-xs sm:text-sm ${data.gainLoss >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                              {data.gainLoss >= 0 ? '+' : ''}${data.gainLoss.toFixed(2)}
                            </p>
                            <p className={`text-xs sm:text-sm ${data.gainLossPercentage >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <button
            onClick={() => navigate('/crypto')}
            className="bg-gradient-to-r from-purple-500 to-purple-600 dark:from-purple-600 dark:to-purple-700 rounded-xl shadow-lg p-6 sm:p-8 hover:shadow-2xl transition-all text-left transform hover:scale-105 active:scale-95"
          >
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-2">Crypto Portfolio</h2>
            <p className="text-sm sm:text-base text-purple-100 dark:text-purple-200">View and manage your cryptocurrency investments</p>
          </button>

          <button
            onClick={() => navigate('/stocks')}
            className="bg-gradient-to-r from-indigo-500 to-indigo-600 dark:from-indigo-600 dark:to-indigo-700 rounded-xl shadow-lg p-6 sm:p-8 hover:shadow-2xl transition-all text-left transform hover:scale-105 active:scale-95"
          >
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-2">Stock Portfolio</h2>
            <p className="text-sm sm:text-base text-indigo-100 dark:text-indigo-200">View and manage your stock investments</p>
          </button>
        </div>
      </main>
    </div>
