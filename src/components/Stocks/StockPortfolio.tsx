...
export function StockPortfolio() {
  ...
  useRegisterAssets(portfolio?.assets || [], 'stock');

  // Load saved state from localStorage
  useEffect(() => {
    const savedFilters = localStorage.getItem('stock-filters');
    const savedPresets = localStorage.getItem('stock-filter-presets');
    const savedQuickFilter = localStorage.getItem('stock-quick-filter');
  ...
  useEffect(() => {
    localStorage.setItem('stock-filters', JSON.stringify(filters));
  }, [filters]);

  // Save quick filter to localStorage
  useEffect(() => {
    localStorage.setItem('stock-quick-filter', quickFilter);
  }, [quickFilter]);
  ...
  const savePreset = useCallback(() => {
    ...
    localStorage.setItem('stock-filter-presets', JSON.stringify(updatedPresets));
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="pl-10 pr-10 py-2 w-64 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
                      />
                      {searchQuery && (
                        <button
                    ...
                    >
                      ...
                      {getActiveFilterSummary().length > 0 && (
                        <span className="ml-1 px-2 py-0.5 text-xs bg-white dark:bg-gray-800 text-indigo-600 dark:text-indigo-400 rounded-full">
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
                      quickFilter === 'highValue'
                        ? 'bg-indigo-600 text-white'
                        : 'bg-indigo-100 hover:bg-indigo-200 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300'
                    }`}
                  >
                    High Value (&gt;$1000)
                  </button>
                ...
                {showSavePreset && (
                  <div className="mb-3 flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm"
                    />
                    <button
                      onClick={savePreset}
                ...
                <div className="flex flex-wrap gap-2">
                  {PRESET_FILTERS.map((preset, index) => (
                    <button
                      ...
                      className="px-3 py-1.5 bg-indigo-100 hover:bg-indigo-200 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 rounded-lg text-sm transition-colors"
                    >
                      {preset.name}
                    </button>
                  ))}
                  {customPresets.map((preset, index) => (
                    <div key={index} className="flex items-center gap-1 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
                      <button
                        onClick={() => loadPreset(preset)}
                        className="px-3 py-1.5 text-blue-700 dark:text-blue-300 text-sm hover:bg-blue-200 dark:hover:bg-blue-900/50 rounded-l-lg transition-colors"
                      >
                        {preset.name}
                      </button>
                      <button
                        onClick={() => deletePreset(index)}
                        className="px-2 py-1.5 text-blue-700 dark:text-blue-300 hover:text-red-600 dark:hover:text-red-400 rounded-r-lg transition-colors"
                        title="Delete preset"
                      >
                        <X className="h-3 w-3" />
                      </button>
